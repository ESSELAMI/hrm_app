        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    v-model="newRole" required v-on:input="errors.name=null"/>
    ...
    <div class="modal fade" id="edit-role-modal" tabindex="-1" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <input type="text" :class="[errors.name ? 'form-control form-control-danger' : 'form-control form-control-success']"
                maxlength="25" required v-on:input="errors.name=null" :placeholder="selectedRoleName"
                v-model="roleName"/>
                <p class="text-danger m-t-5" v-if="errors.name" >@{{errors.name.toString()}}</p>
                </div>
                <div class="modal-footer">
                ...
                <div class="dt-responsive table-responsive">
                    ...
                    <table id="roles-table" class="table table-hover table-bordered nowrap">
                        ...
                        <tbody>
                            ...
                            :class="{'selected-row': selectedRoleName === role.name}"
                            v-on:click="showPermissions(role, role.permissions),selectedRoleIndex = index">
                                <td>@{{ index+1}}</td>
                                <td>@{{ role.name }}</td>
                                <td>
                    ...
                    <ul class="list-unstyled card-option">
                        <li>
                            <span data-toggle="tooltip" data-placement="left"
                                  data-original-title="Attribuer des permissions">
                                    <i class="feather icon-plus text-success md-trigger" data-toggle="modal"
                                        data-target="#assign-permissions-modal"
                                        v-on:click="get_selected_permissions()">
                                    </i>
                            </span>
                        </li>
                    </ul>
                            ...
                            <tr v-for="(permission, index) in role_permissions" :key="index" >
                                ...
                                <td>
                                    <div class="text-center">
                                        ...
                                        data-original-title="Supprimer" v-on:click="revoke_permission(permission.id,index)">
                                        </i>
                                    </div>
                                </td>
                            </tr>
...
const app = new Vue({
  el: '#app',
        ...
        methods:{
                ...
                return axios.get('/getPermissions')
                        ...
                        .then(function(response){
                            ...
                            this.permissions.forEach(permission => {
                                $('#test').multiSelect(
                                'addOption', { value: permission.id, text: permission.name },
                                );
                            });
                        })
                        .catch();
            ...
            add_role(){
                axios.post('/role_add', {'name':app.newRole})
                .then(function (response) {
            ...
            update_role(name,index){
                axios.put('/role_edit/'+this.selectedRole, {'name':name})
                .then(function (response) {
            ...
            assign_permissions(){
                ...
                axios.post('/role_assign_permissions/'+this.selectedRole, {'permissions':permissions})
                .then(function (response) {
                   // app.$set(app.roles,index,response.data.role);
                   // app.fetch_roles();
            ...
            revoke_permission(id,index){
                ...
                axios.post('/role_revoke_permission/'+this.selectedRole, {'permission':id})
                .then(function (response) {
